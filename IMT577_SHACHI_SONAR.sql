...
CREATE OR REPLACE TABLE Dim_Product (
    DimProductID INTEGER IDENTITY(1,1) CONSTRAINT PK_dimProduct PRIMARY KEY NOT NULL, -- SurrogateKey
	...
	ProductWholesaleUnit FLOAT NOT NULL,
...
SELECT
	...
	Product.Cost,
	(Product.Price - Product.Cost) as RetailUnitProfit,
	(Product.Wholesaleprice - Product.Cost) as WholesaleUnitProfit,
	((Product.Price - Product.Cost)/Product.Price)*100 as Margin
from Product
INNER JOIN  ProductType
ON Product.ProductTypeID = ProductType.ProductTypeID
INNER JOIN ProductCategory 
ON ProductType.ProductcategoryID = ProductCategory.ProductcategoryID;
...
CREATE OR REPLACE TABLE Dim_location (
    DimLocationID INT IDENTITY(1,1) CONSTRAINT PK_DimCharacterID PRIMARY KEY NOT NULL,--Surrogate Key
...
ON
Store.Address = Dim_location.Address;
...
select
	...
	Reseller.EmailAddress
from Reseller
INNER JOIN Dim_location
ON
Reseller.Address = Dim_location.Address;
...
SELECT 
    ...
    CONCAT(Customer.FirstName,SPACE(1),Customer.LastName) AS CustomerFullName,
    Customer.FirstName AS FirstName,
    Customer.LastName AS LastName,
    Customer.Gender AS CustomerGender
FROM Customer
INNER JOIN Dim_Location ON
Dim_Location.Address = Customer.Address;
...
CREATE OR REPLACE TABLE Dim_channel (
    DimChannelID INTEGER IDENTITY(1,1) CONSTRAINT PK_dimChannel PRIMARY KEY NOT NULL ,-- SurrogateKey
...
SELECT
	Channel.ChannelID,
	ChannelCategory.ChannelCategoryID,
	Channel.Channel,
	ChannelCategory.ChannelCategory
from Channel
INNER JOIN ChannelCategory
ON
Channel.ChannelCategoryID = ChannelCategory.ChannelCategoryID;
